 ...
 * 快手可灵 AI API 客户端
 * 
 * 文档：https://platform.kuaishou.com/docs
 */
...
export interface KelingClientConfig {
  apiKey: string;
  apiSecret: string;
  baseUrl: string;
  timeout?: number;
...
export class KelingClient {
  ...
  constructor(config: KelingClientConfig) {
    this.apiKey = config.apiKey;
    this.apiSecret = config.apiSecret;
    this.logger = new Logger('KelingClient');
    ...
    this.client.interceptors.request.use((config) => {
      // 这里可以根据实际 API 要求添加认证头
      // 示例：config.headers['Authorization'] = `Bearer ${this.apiKey}`;
  ...
  async generateVideo(request: KelingVideoRequest): Promise<KelingVideoResponse> {
    ...
    try {
      const response = await this.client.post<KelingVideoResponse>(
        '/v1/video/generate',
        request
      );
  ...
  async queryTask(taskId: string): Promise<KelingVideoResponse> {
    ...
    try {
      const response = await this.client.get<KelingVideoResponse>(
   ...
   * 构建参考图参数
   */
  static buildReferenceParams(referenceImages: {
    character?: string;
    scene?: string;
  }): { reference_image?: string; reference_type?: 'character' | 'scene' | 'both' } {
    const { character, scene } = referenceImages;

    if (character && scene) {
      // 如果都有，优先使用人物参考图（可灵可能只支持一张参考图）
      return {
